...
$h->b('div', 0, 1, '{"class":"row", "data-min-height":"50"}');
	$h->b('div', 0, 1, '', '{"class":"vertical"}');
		$h->b('div', 0, 1, '', '{"class":"col col-center col-base-8"}');
			...
			$h->b('section', 0, 1, '{"class":"col col-center col-base-12"}');
				foreach($data as $key => $val){
					$h->b('figure', 0, 1, '{"class":"col col-xs-12 col-lg-4"}');
						$h->b('div', 0, 1);
							$h->b('a', 0, 1, '', '{"href":"'.trim($text[$data[0]]).'", "class":"col col-xs-12 col-sm-4"}');
								$h->b('i', 0, 1, '', '{"class":"'.trim($text[$data[1]]).'"}');
								$h->b('i', 1, 1);
							$h->b('a', 1, 1);
							$h->b('figcaption', 0, 1, '{"class":"col col-xs-12 col-sm-8 vertical"}');
								$h->b('p', 0, 1);
									$h->e(1, $text[$data[2]]);
								$h->b('p', 1, 1);
							$h->b('figcaption', 1, 1);
